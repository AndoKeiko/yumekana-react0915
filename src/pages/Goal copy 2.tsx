...
const Goal: React.FC<GoalProps> = () => {
  ...
  //   }
  // };
  const handleSave = async (id: string | number, updatedTask: Task) => {
    try {
      const response = await axios.put<Task>(API_ENDPOINTS.UPDATE_TASK(Number(id)), updatedTask);
      const savedTask: Task = response.data;
      setTasks(prevTasks => prevTasks.map(task =>
        task.id === Number(id) ? { ...task, ...savedTask } : task
      ));
    } catch (error) {
      console.error("Failed to update task:", error);
      setServerError("タスクの更新に失敗しました");
  ...
  //   }
  // };
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>, field: keyof Task, taskId: number) => {
